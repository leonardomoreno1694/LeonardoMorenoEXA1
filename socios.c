...
void ordenarPrestamosPorFecha (e_Prestamos* prestamo, int cantidadPrestamos)
{

    int i, j ;
    e_Prestamos prestamoAUX;
    for(i=0; i<cantidadPrestamos; i++)
    {
        for(j = i + 1; j<cantidadPrestamos; j++)
        {

            if(prestamo[i]. fechaPrestamo.anio > prestamo[j]. fechaPrestamo.anio)
            {
            ...
            if ( prestamo[i]. fechaPrestamo.anio ==  prestamo[j]. fechaPrestamo.anio)
            {
                ...
                if(prestamo[i]. fechaPrestamo.mes > prestamo[j]. fechaPrestamo.mes)
                {
                    prestamoAUX = prestamo[j];
                    prestamo[j] = prestamo[i];
                    prestamo[i] = prestamoAUX;
            ...
            if (prestamo[i]. fechaPrestamo.mes == prestamo[j]. fechaPrestamo.mes)
            {
                if(prestamo[i]. fechaPrestamo.dia > prestamo[j]. fechaPrestamo.dia)
                {
                    prestamoAUX = prestamo[j];
                    prestamo[j] = prestamo[i];
                    prestamo[i] = prestamoAUX;
...
//**------------------------------------------------------------------------------------------------**//
float buscarPromedioDiarioPrestamos (e_Prestamos prestamo [], int cantidadPrestamos)
{
...
int buscarCantidadPrestamosCargados ( e_Prestamos prestamo[],int cantidadPrestamos)
{
...
int buscarCantidadDiasUnicosPrestamos(e_Prestamos prestamo[], int cantidadPrestamos )
{
    ...
    {
        if (    (prestamo[i].fechaPrestamo.anio != prestamo[i+1].fechaPrestamo.anio) ||
                (prestamo[i].fechaPrestamo.mes != prestamo[i+1].fechaPrestamo.mes) ||
                (prestamo[i].fechaPrestamo.dia != prestamo[i+1].fechaPrestamo.dia) )
        {
            diasUnicos ++ ;
        };
    }
...
void listarTotalYPromedio(e_Prestamos prestamo[],int cantidadPrestamos)
{
...
void listarDiasPorDebajoPromedio(e_Prestamos prestamo [], int cantidadPrestamos )
{
        ...
        if(prestamo[i].isEmpty == 0)
        {
            ...
            for(j = i ; j<cantidadPrestamos ; j++)
            {
                if(prestamo[j].isEmpty == 0)
                {
                    ...
                    if(  (prestamo[i].fechaPrestamo.dia == prestamo[j].fechaPrestamo.dia)&&
                            (prestamo[i].fechaPrestamo.anio == prestamo[j].fechaPrestamo.anio)&&
                            (prestamo[i].fechaPrestamo.mes == prestamo[j].fechaPrestamo.mes)  )
                    {
                        prestamosEseDia ++ ;
                    }
                    else
